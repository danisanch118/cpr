...
class BoatController(Node):
    ...
    def odom_callback(self,msg):
        ...
        self.raw_w = msg.twist.twist.angular.z    
    ...
    def control_loop(self):
        ...
        # 0. Filtrado
        self.current_w = (self.alpha_w * self.raw_w) + ((1.0 - self.alpha_w) * self.current_w)
        ...
        if ((abs(self.ref_v)<0.2) and (abs(error_v)<0.2)):
            E = 0
	    
        # Salida PI Angular (C_M del diagrama)
        M = (KP_W * error_w) + (KI_W * self.integral_error_w)
        if ((abs(self.ref_w)<0.2) and (abs(error_w)<0.2)):
            M = 0
